...
contract VaultSecureTest is Test {
    ...
    function test_GasConsumptionAttack_SecureVault() public {
        ...
        // 验证恶意合约仍然有余额（因为转账失败）
        assertEq(vaultSecure.balances(address(maliciousContract)), 1 ether, "Malicious contract should still have balance");
    }
    ...
    function test_ContractDetection() public {
        ...
        assertEq(vaultSecure.isContract(address(gasConsumingContract)), true, "GasConsumingContract should be a contract");
    }
    
    function test_ContractSizeDetection() public {
        // 测试合约大小检测
        assertEq(vaultSecure.isContractTooLarge(address(maliciousContract)), false, "MaliciousContract should not be too large");
        assertEq(vaultSecure.isContractTooLarge(address(gasConsumingContract)), false, "GasConsumingContract should not be too large");
    }
