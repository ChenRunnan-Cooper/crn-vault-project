...
contract ReturnBombTest is Test {
    ...
    function test_ReturnBombAttack_SecureVault() public {
        ...
        // 验证余额没有减少（因为转账失败）
        assertEq(vaultSecure.balances(address(returnBombContract)), 1 ether, "Balance should remain unchanged");
    }
    ...
    function test_LargeReturnDataAttack_SecureVault() public {
        ...
        // 验证余额没有减少（因为转账失败）
        assertEq(vaultSecure.balances(address(largeReturnDataContract)), 1 ether, "Balance should remain unchanged");
    }
    ...
    function test_NormalContract_SecureVault() public {
        ...
        assertEq(vaultSecure.balances(address(normalContract)), 1 ether, "Balance should remain unchanged");
    }
    ...
    function test_ContractDetection_ReturnBomb() public {
        // 测试合约检测功能
        assertEq(vaultSecure.isContract(address(returnBombContract)), true, "ReturnBombContract should be detected as contract");
        assertEq(vaultSecure.isContract(address(largeReturnDataContract)), true, "LargeReturnDataContract should be detected as contract");
